...
class CompanyAdRepository
{
    ...
    {
        $sql = "SELECT c.*,ca.*,u.user_status FROM company c JOIN company_ad ca ON c.user_id=ca.company_id JOIN users u ON u.user_id=c.user_id WHERE c.company_name like CONCAT('%', '{$query}', '%')";
        $result = $this->conn->query($sql);
        $list = [];
        while ($row = $result->fetch_assoc()) {
            $company = new MyCompanyModel($row["user_id"], $row["company_name"], $row["email"], $row["contact_no"], $row["contact_person"], $row["company_site"], $row["user_status"]);
