...
class StudentRepository{
    ...
    public function findAllByRoundId($roundId): array{
        // $sql = "SELECT s.user_id,s.email,s.first_name,s.last_name,s.reg_no,s.index_no,a.applied_date,c.* FROM student s 
        //         JOIN applyadvertisement a ON s.user_id = a.applied_by 
        //         JOIN firstrounddata f ON a.id=f.applied_id 
        //         JOIN company_ad c ON c.ad_id=f.ad_id WHERE a.round_id = $roundId";
        ...
        while($row=$result->fetch_assoc())
        {
            ...
            while($r=$companyResult->fetch_assoc())
            {
                ...
                $ad=new CompanyAdModel($r["ad_id"],$r["position"],$r["requirements"],$r["no_of_intern"],$r["working_mode"],$r["from_date"],$r["to_date"],$r["company_id"],$r["qualification"],$company);
                $adList[]=$ad;
            }
